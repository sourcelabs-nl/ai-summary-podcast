...
class PollDelayResolverTest {
    ...
    fun `type default is used when no per-source or host override`() {
        ...
        assertEquals(2, r.resolveDelaySeconds(source(type = "website", url = "https://example.com")))
    }
    ...
    fun `unparseable URL falls through to type default`() {
        ...
        assertEquals(4, r.resolveDelaySeconds(source(url = "not a url")))
    }

    @Test
    fun `unparseable URL with no type default returns 0`() {
        val r = resolver()
        assertEquals(0, r.resolveDelaySeconds(source(url = "not a url")))
    }
