...
class SourcePollerFailureTrackingTest {
    ...
    fun `increments consecutiveFailures on transient error`() {
        every { rssFeedFetcher.fetch(any(), any(), any()) } throws RuntimeException("timeout")
    ...
    fun `increments consecutiveFailures on permanent error`() {
        every { rssFeedFetcher.fetch(any(), any(), any()) } throws HttpClientErrorException(HttpStatus.NOT_FOUND)
    ...
    fun `resets failure tracking on success`() {
        ...
        every { rssFeedFetcher.fetch(any(), any(), any()) } returns listOf(post)
