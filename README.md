...
Source list responses include `articleCount` and `relevantArticleCount` fields — the total number of articles collected from the source and how many scored at or above the podcast's `relevanceThreshold`. Counts are computed in a single batch query for efficiency.
