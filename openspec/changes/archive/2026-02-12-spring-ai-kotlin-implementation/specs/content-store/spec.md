...
The system SHALL persist episodes in an `episodes` SQLite table with columns: `id` (auto-generated), `generated_at` (timestamp), `script_text` (text), `audio_file_path` (text), and `duration_seconds` (integer). Each episode represents one generated audio briefing.

#### Scenario: Episode stored after successful generation
- **WHEN** the TTS pipeline completes and produces an MP3 file
- **THEN** an episode record is created with the script text, audio file path, and calculated duration

### Requirement: SQLite database initialization
The system SHALL create the SQLite database file and all required tables on first startup if they do not exist. The database SHALL use WAL mode for improved read concurrency.

#### Scenario: First startup creates schema
- **WHEN** the application starts and no database file exists
- **THEN** the database file is created with `sources`, `articles`, and `episodes` tables

#### Scenario: Subsequent startup preserves data
- **WHEN** the application starts and the database already exists with data
- **THEN** existing data is preserved and no schema changes are applied
