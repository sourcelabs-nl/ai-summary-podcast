...
#### Scenario: TTS model identifier included in result
- **WHEN** the OpenAI TTS provider generates audio
- **THEN** the TTS result includes `model = "tts-1"`

## ADDED Requirements

### Requirement: TTS model stored on episode
The system SHALL store the TTS model identifier on the episode after audio generation. The `TtsResult` SHALL include a `model` field containing the model identifier used by the provider. The `TtsPipeline` SHALL persist this value as `ttsModel` on the episode entity. The `tts_model` column SHALL be nullable to support episodes generated before this change.

#### Scenario: TTS model persisted on new episode
- **WHEN** `TtsPipeline.generate()` creates a new episode
- **THEN** the episode's `ttsModel` field contains the model identifier from the TTS result

#### Scenario: TTS model persisted on existing episode
- **WHEN** `TtsPipeline.generateForExistingEpisode()` generates audio for an approved episode
- **THEN** the episode's `ttsModel` field contains the model identifier from the TTS result

#### Scenario: ElevenLabs model identifier
- **WHEN** the ElevenLabs TTS provider generates audio
- **THEN** the TTS result includes `model = "eleven_flash_v2_5"`

#### Scenario: Legacy episode without TTS model
- **WHEN** an episode was generated before this change
- **THEN** the `ttsModel` field is null
