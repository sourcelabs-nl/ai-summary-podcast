...
Stage 1 (scoring) SHALL query articles where `relevance_score IS NULL` for the podcast's sources, score each article 0-10, and persist the score immediately. Stage 2 (summarization) SHALL query articles where `relevance_score >= podcast.relevanceThreshold` AND `summary IS NULL`, skip articles whose body word count is below the global `app.llm.summarization-min-words` threshold, and summarize the remaining articles. Stage 3 (composition) SHALL query articles where `relevance_score >= podcast.relevanceThreshold` AND `is_processed = false`, and compose a briefing script.
...
- **THEN** articles are scored (0-10), relevant articles are conditionally summarized, and a briefing script is produced
