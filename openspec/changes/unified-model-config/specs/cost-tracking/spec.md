...
### Requirement: TTS cost estimation
**Reason**: Replaced by unified model config lookup instead of separate `app.tts.cost-per-million-chars` map
**Migration**: TTS costs are now defined under `app.models.<provider>.<model>.cost-per-million-chars`

### Requirement: Model pricing configuration
**Reason**: Replaced by unified `app.models` structure that covers both LLM and TTS costs
**Migration**: LLM costs move from `app.llm.models.<alias>` to `app.models.<provider>.<model>`. TTS costs move from `app.tts.cost-per-million-chars` to `app.models.<provider>.<model>.cost-per-million-chars`.
