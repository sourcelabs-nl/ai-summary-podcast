...
- **WHEN** a `DELETE /users/{userId}/api-keys/{category}/{provider}` request is received with a `category` that is not `LLM`, `TTS`, or `PUBLISHING`
- **THEN** the system returns HTTP 400 with a validation error message
